  ...
  <v-card :class="$style.dialog">
    ...
    <v-card-text>
      <v-container grid-list-md>
        <v-layout wrap>
          <v-flex xs12>
            <v-btn color="indigo" block dark large :class="$style.snslogin" @click="callAuth({ providerName: 'facebook'  })">Facebookアカウントで作成</v-btn>
            <v-btn disabled color="light-blue" block dark large :class="$style.snslogin" to="/user">Twitterアカウントで作成</v-btn>
            <v-btn color="blue" block dark large :class="$style.snslogin" @click="callAuth({ providerName: 'google' })">Googleアカウントで作成</v-btn>
          </v-flex>
